"""
LIFTROI DATA QUALITY FUNCTIONS v1.1
...
def media_list_namer(input_string):
    space_pattern = r'::'
    space_remover = r'\s'
...
def per_media_df_generator(dataframe, hierarchy_df):
    ...
    for media in media_list:
        ...
        media_dataframes.append((media, temp_df))
...
def cost_per_activity_creator(list_of_per_media_frames):
    ...
    for i_media in range(len(list_of_per_media_frames)):
        ...
        spend_col_name = [name for name in list_of_per_media_frames[i_media][1].columns if '$' in name]
    ...
    spend_cols = [name for name in result_df.columns if '$' in name]
